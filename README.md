...
[release]: https://github.com/zhangguanzhang/process-exporter/releases/latest

[![Release](https://img.shields.io/github/release/zhangguanzhang/process-exporter.svg?style=flat-square")][release]
...
![Build](https://github.com/zhangguanzhang/process-exporter/actions/workflows/build.yml/badge.svg)
...
install via [docker](https://hub.docker.com/r/zhangguanzhang/process-exporter/).
...
docker run -d --rm -p 9256:9256 --privileged \
  -v /proc:/host/proc -v `pwd`:/config zhangguanzhang/process-exporter \
...
```
docker run --rm -ti -v $PWD:/v -w /v registry.aliyuncs.com/zhangguanzhang/loong64:go1.21 make build ARCH=loong64
DOCKER_BUILDKIT=0 DOCKER_BUILD_ARGS="-f Dockerfile.local --platform=linux/loong64" make docker
```
